...
export default function LanguageForm() {
  ...
  useEffect(() => {
    ...
    axios.get(API_BASE_URL + "/typeLanguages")
    .then((response) => {
      setTypeLanguages(response.data)
      if(response.data.length > 0) {
        setIdTypeLanguage(response.data[0].idTypeLanguage)
      }
    })
    .catch((error) => {
      alert(error)
    ...
    axios.get(API_BASE_URL + "/payss")
    .then((response) => {
      setPayss(response.data)
      if(response.data.length > 0) {
        setIdPays(response.data[0].idPays)
      }
    })
    .catch((error) => {
      alert(error)
  ...
  const saveLanguage = (e) => {
    ...
    if (id === undefined) {
      axios
      .post(API_BASE_URL + "/languages", newLanguage)
        .then((response) => {
          navigate("/languages");
        })
        .catch((error) => {
      ...
      axios
        .put(API_BASE_URL + "/languages/" + id, newLanguage)
        .then((response) => {
          navigate("/languages");
        })
        .catch((error) => {
