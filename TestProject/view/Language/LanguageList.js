...
export default function LanguageList() {
  ...
  useEffect(() => {
    axios.get(API_BASE_URL + "/languages?page=" + currentPage + "&size=5", {headers: {"Authorization": "Bearer " + localStorage.getItem("apktoken")}}).then((response) => {
      if (totalPage === 0) {
  ...
  const deleteLanguage = (idLanguage) => {
    axios.delete(API_BASE_URL + "/languages/" + idLanguage, {headers: {"Authorization": "Bearer " + localStorage.getItem("apktoken")}}).then((response) => {
