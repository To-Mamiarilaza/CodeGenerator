...
public class PaysController  {
    ...
    @GetMapping
    public ResponseEntity<Page<Pays>> getAllPayss(@RequestParam(name = "page",defaultValue = "0") Integer page, @RequestParam(name = "size",defaultValue = "5") Integer size, HttpServletRequest request) {
        try {
    ...
    @GetMapping("/{idPays}")
    public ResponseEntity<Pays> getPaysById(@PathVariable Integer idPays, HttpServletRequest request) {
        try {
            JWTToken.checkBearer(request);
            } catch(NotAuthorizedException e) {
                return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        ...
        @PostMapping
        public ResponseEntity<Pays> createPays(@RequestBody Pays pays, HttpServletRequest request) {
            try {
                JWTToken.checkBearer(request);
        ...
        @PutMapping("/{idPays}")
        public ResponseEntity<Pays> updatePays(@PathVariable Integer idPays, @RequestBody Pays pays, HttpServletRequest request) {
            try{
                JWTToken.checkBearer(request);
        ...
        public ResponseEntity<Void> deletePays(@PathVariable Integer idPays, HttpServletRequest request) {
            try {
                JWTToken.checkBearer(request);
